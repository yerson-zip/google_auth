...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    user_register = await google_info(request)
    user = RegisterUser(email=user_register["email"],
                        full_name=user_register["full_name"],
                        picture=user_register["picture"])

    return login_service(db=db, user_request=user)
